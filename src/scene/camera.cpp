...
ViewportPoint Camera::local_to_viewport(const Vec &local_point) const {
    ...
    return {BufferCoord(row, col), depth};
}
...
Ray Camera::viewport_to_local_ray(const BufferCoord &viewport_point, Real row_offset, Real col_offset) const {
    Real x = (viewport_point.col + col_offset - width * 0.5) * 0.5;
    Real y = -viewport_point.row - row_offset + height * 0.5;
    return {0, Vec(x, y, lens)};
}

Ray Camera::viewport_to_world_ray(const BufferCoord &viewport_point, Real row_offset, Real col_offset) const {
